...
$char_limit = $config['news_char_limit'];
...
$sql = 'SELECT t.*, f.*, p.*, s.*, u.*, z.*
	...
	LEFT JOIN " . ZEBRA_TABLE . " z
		on z.user_id = " . $user->data['user_id'] . "
			AND z.zebra_id = p.poster_id
	WHERE t.forum_id IN ($forums)
		AND $forum_sql" . 
		(($only_news) ? " AND t.topic_id = $only_news" : "") . "
	GROUP BY t.topic_id, s.session_user_id
...
while ($row = $db->sql_fetchrow($result))
{
	...
	$row['post_text'] = ((utf8_strlen($row['post_text']) > $char_limit + 50) && !$only_news) ? (utf8_substr($row['post_text'],0,$char_limit) . '...') : $row['post_text'];
	...
	$template->assign_block_vars('postrow', array(
		...
		'U_NEWS'				=> append_sid("{$phpbb_root_path}newspage.$phpEx", 'news=' . $topic_id),
